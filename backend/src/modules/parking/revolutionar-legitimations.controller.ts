...
export class RevolutionarLegitimationsController {
  ...
  @Post()
  async create(@Request() req, @Body() dto: CreateRevolutionarLegitimationDto) {
  ...
  async findAll(
    @Request() req,
    @Query('status') status?: RevolutionarLegitimationStatus,
  ) {
    this.checkAccess(req.user);
  ...
  async update(
    @Request() req,
    @Param('id') id: string,
    @Body() dto: UpdateRevolutionarLegitimationDto,
  ) {
    this.checkAccess(req.user);
