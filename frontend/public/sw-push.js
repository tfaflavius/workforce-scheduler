...
// Most browsers fire this when the subscription is regenerated by push services.
// We must:
//  1. Fetch the VAPID public key from the API (no auth needed — endpoint is @Public)
//  2. Re-subscribe with that key
//  3. Notify any open client(s) so they can POST the new subscription to the
//     server with auth (the subscribe endpoint requires JWT)
//  4. If no client is open, the notification will arrive next time the user
//     opens the app and the page-side flow will sync with the server.
  ...
  event.waitUntil(
    (async () => {
      try {
        ...
        const vapidResp = await fetch(API_BASE + '/notifications/push/vapid-public-key');
        if (!vapidResp.ok) {
          console.error('[SW] Failed to fetch VAPID key, status:', vapidResp.status);
          return;
        }
        const { publicKey } = await vapidResp.json();
        if (!publicKey) {
          console.error('[SW] VAPID key empty in response');
          return;
        }

        // 2. Re-subscribe with the fetched key
        const subscription = await self.registration.pushManager.subscribe({
          userVisibleOnly: true,
          applicationServerKey: urlBase64ToUint8Array(publicKey),
        });
        console.log('[SW] New subscription obtained:', subscription.endpoint.substring(0, 50));

        // 3. Notify all open clients so the page can register the new subscription with auth.
        ...
        console.log(`[SW] Notified ${allClients.length} clients of new subscription`);
      } catch (err) {
