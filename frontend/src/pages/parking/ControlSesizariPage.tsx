...
const CreateControlSesizareDialog: React.FC<CreateDialogProps> = ({ open, onClose, sesizareType }) => {
  ...
  // Auto-detect GPS on dialog open
  useEffect(() => {
  ...
  const generateMapsLinkFromCoords = (lat: number, lng: number) => {
  ...
  const handleGetGpsLocation = () => {
    if (!navigator.geolocation) {
      setError('Browserul nu suporta geolocalizarea');
      return;
    }
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        // Generate Google Maps link from exact coordinates
        const mapsLink = generateMapsLinkFromCoords(latitude, longitude);

        // Reverse geocode to get street address
        try {
          const response = await fetch(
            `https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitude}&lon=${longitude}&zoom=18&addressdetails=1&accept-language=ro`
          );
          ...
          if (data.address) {
            ...
            if (road) {
              locationText = houseNumber ? `${road} nr. ${houseNumber}` : road;
            } else {
              locationText = data.display_name?.split(',').slice(0, 2).join(',').trim() || `${latitude.toFixed(6)}, ${longitude.toFixed(6)}`;
            }
          } else {
            locationText = `${latitude.toFixed(6)}, ${longitude.toFixed(6)}`;
          }

          const cleanLocation = removeDiacritics(locationText);
          setFormData(prev => ({
            ...prev,
      ...
      (err) => {
        ...
        if (err.code === err.PERMISSION_DENIED) {
          setError('Acces la locatie refuzat. Permiteti accesul in setarile browserului.');
        } else if (err.code === err.POSITION_UNAVAILABLE) {
          setError('Locatia nu este disponibila. Verificati GPS-ul.');
        } else {
          setError('Nu s-a putut obtine locatia. Incercati din nou.');
        }
      },
  ...
  const handleLocationChange = (value: string) => {
    ...
    // If user edits location manually and we had GPS coords, keep the GPS maps link
    // Otherwise auto-generate from text
    if (gpsCoords) {
      setFormData(prev => ({
        ...prev,
        location: cleanValue,
      ...
      setFormData(prev => ({
        ...prev,
        location: cleanValue,
        googleMapsLink: generateMapsLink(cleanValue),
      }));
    }
  };
    ...
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth fullScreen={isMobile}
      ...
      <DialogContent dividers sx={{ p: { xs: 2, sm: 3 }, pt: '20px !important' }}>
        <Stack spacing={2.5}>
          ...
          {/* GPS Location Button */}
          <Button
            variant={gpsCoords ? 'contained' : 'outlined'}
            ...
            sx={{
              ...
              ...(gpsCoords ? {
                ...
                color: typeColor,
                borderWidth: 2,
                '&:hover': { borderWidth: 2, borderColor: typeColor, bgcolor: alpha(typeColor, 0.06) },
              }),
            }}
          >
            {gpsLoading ? 'Se obtine locatia...' : gpsCoords ? 'Locatie detectata ✓ (Apasa pentru redetectare)' : 'Detecteaza locatia mea (GPS)'}
          </Button>
          ...
          <TextField
            ...
            placeholder={gpsLoading ? 'Se detecteaza...' : 'Ex: Str. Republicii nr. 10'}
            helperText={gpsCoords ? 'Locatia a fost completata automat din GPS. Puteti modifica manual.' : 'Apasati butonul de mai sus pentru detectare automata'}
            InputProps={{
              startAdornment: <InputAdornment position="start"><PlaceIcon color={gpsCoords ? 'success' : 'action'} /></InputAdornment>,
            }}
          />
